...
__global__ void kernel_quaternion_mul(
	...
	// locate
	// (B / R1) x D1 is broadcast as B/R1 x R1 x D1
	inputs_1 += (b / R1) * D1;
	inputs_2 += (b / R2) * D2;
	outputs += b * 4;
...
__global__ void kernel_quaternion_mul_backward(
	...
	grad_inputs_1 += D1 * (b / R1) + d + D1 - 4;
	grad_inputs_2 += D2 * (b / R2) + d + D2 - 4;
	if (d==0) {
		if (D1 > 3){
...
__global__ void kernel_quaternion_mul_backward_backward(
	...
	grad_grad_inputs_1 += D1 * (b/R1) + d + D1 - 4;
	grad_grad_inputs_2 += D2 * (b/R2) + d + D2 - 4;
...
// outputs: [B, L * C], float
template <typename scalar_t>
void quaternion_mul_forward_cuda(const scalar_t *inputs_1, const scalar_t *inputs_2, scalar_t *outputs, const uint32_t B, const uint32_t R1, const uint32_t R2, const uint32_t D1, const uint32_t D2) {
	static constexpr uint32_t N_THREADS = 256;
	kernel_quaternion_mul<scalar_t><<<div_round_up(B, N_THREADS), N_THREADS>>>(inputs_1, inputs_2, outputs, B, R1, R2, D1, D2);
}


template <typename scalar_t>
void quaternion_mul_backward_cuda(const scalar_t *grad, const uint32_t B, const uint32_t R1, const uint32_t R2, const uint32_t D1, const uint32_t D2, const scalar_t *inputs_1, const scalar_t *inputs_2, scalar_t *grad_inputs_1, scalar_t *grad_inputs_2) {
	static constexpr uint32_t N_THREADS = 256;
	kernel_quaternion_mul_backward<scalar_t><<<div_round_up(B * 4, N_THREADS), N_THREADS>>>(grad, B, R1, R2, D1, D2, inputs_1, inputs_2, grad_inputs_1, grad_inputs_2);
}
...
void quaternion_mul_backward_backward_cuda(
	...
	kernel_quaternion_mul_backward_backward<scalar_t><<<div_round_up(B * 4, N_THREADS), N_THREADS>>>(
		grad_out_1, grad_out_2, B, R1, R2, D1, D2, 
